# Taller independiente — Clase 14
**DML: SELECT, filtros y funciones de agregación | Base de datos: AeroPass**
...
> Copia y ejecuta el bloque de preparación en pgAdmin antes de comenzar los ejercicios.
...
**5.** ¿Qué pasajeros no tienen email registrado? Muestra su código y nombre. *(IS NULL)*

**6.** Lista los vuelos cuyo destino sea `'Bogotá'` o `'Medellín'`. *(IN)*

**7.** Lista las reservas realizadas entre el 15 y el 31 de agosto de 2024 (incluyente). *(BETWEEN)*

**8.** Lista los pasajeros cuyo nombre empiece con la letra `'A'`, sin importar mayúsculas. *(ILIKE)*

**9.** Lista las reservas que **no** sean de clase `'Económica'`, ordenadas de mayor a menor precio. *(`<>`+ ORDER BY)*

**10.** Lista las reservas de la aerolínea `'AER-1'` que sean en clase `'Económica'`.
*(Pista: primero identifica los `id_vuelo` de AER-1 en la tabla `vuelo`, luego filtra en `reserva` con `IN`)*

**11.** Lista las reservas de septiembre de 2024 en clase `'Ejecutiva'` o `'Primera'`, ordenadas por fecha. *(BETWEEN + IN)*
...
**13.** Lista todos los vuelos ordenados por `origen_vuelo` (A→Z) y, en caso de empate, por `destino_vuelo` (A→Z).
...
**15.** En una sola consulta: precio total, precio promedio (sin decimales), precio máximo y precio mínimo de todas las reservas.
...
**17.** ¿Cuántas reservas hay en cada clase? Ordena de mayor a menor cantidad.
...
**20.** Lista solo los pasajeros con **más de 2 reservas**. *(HAVING)*

**21.** ¿Cuántos vuelos tiene cada aerolínea? Muestra solo las aerolíneas con **más de 2 vuelos**. *(HAVING)*

**22.** ★ ¿Qué clases tienen un precio promedio superior a $500.000 considerando únicamente las reservas de agosto de 2024? Muestra la clase y el promedio.
*(WHERE para el mes + GROUP BY + HAVING)*
